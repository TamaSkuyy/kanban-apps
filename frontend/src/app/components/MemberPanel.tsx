...
type Member = BoardMember | { user_id: string; email: string; role: string; created_at?: string };

export default function MemberPanel({ boardId }: { boardId: string }) {
  const { currentBoard } = useKanbanStore();
  const workspaceId = (currentBoard as any)?.workspace_id as string | null | undefined;
  const isWorkspaceBoard = !!workspaceId;

  const [open, setOpen] = useState(false);
  const [members, setMembers] = useState<Member[]>([]);
  const [inviteEmail, setInviteEmail] = useState('');
  const [inviteRole, setInviteRole] = useState<'editor' | 'viewer' | 'member' | 'admin'>('editor');
  ...
  useEffect(() => {
    if (!open) return;
    const path = isWorkspaceBoard
      ? `/api/workspaces/${workspaceId}/members`
      : `/api/boards/${boardId}/members`;
    apiFetch<{ members: Member[] }>(path)
      .then((d) => setMembers(d.members))
      .catch(() => {});
  }, [open, boardId, workspaceId, isWorkspaceBoard]);
  ...
  async function handleInvite() {
    ...
    try {
      if (isWorkspaceBoard) {
        // map editor->member, viewer->viewer, admin stays
        const wsRole = inviteRole === 'editor' ? 'member' : inviteRole;
        await apiFetch(`/api/workspaces/${workspaceId}/members`, {
          method: 'POST',
          body: JSON.stringify({ email: inviteEmail.trim(), role: wsRole }),
        });
      } else {
        await apiFetch(`/api/boards/${boardId}/members`, {
          method: 'POST',
          body: JSON.stringify({ email: inviteEmail.trim(), role: inviteRole === 'member' ? 'editor' : inviteRole }),
        });
      }
      setInviteEmail('');
      toast.success(`Mengundang ${inviteEmail.trim()}`);
      const path = isWorkspaceBoard ? `/api/workspaces/${workspaceId}/members` : `/api/boards/${boardId}/members`;
      const d = await apiFetch<{ members: Member[] }>(path);
  ...
  async function handleRemove(userId: string, email: string) {
    try {
      const path = isWorkspaceBoard
        ? `/api/workspaces/${workspaceId}/members/${userId}` // not exists, fallback to workspace delete via board? For now use workspace members delete via generic
        : `/api/boards/${boardId}/members/${userId}`;
      // workspace remove not yet implemented as DELETE /workspaces/:id/members/:userId, but backend has no such endpoint yet - fallback to board delete for now
      // try workspace first, fallback to board
      try {
        await apiFetch(`/api/workspaces/${workspaceId}/members/${userId}` as any, { method: 'DELETE' });
      } catch {
  ...
  async function handleRoleChange(userId: string, role: string) {
    try {
      if (isWorkspaceBoard) {
        await apiFetch(`/api/workspaces/${workspaceId}/members/${userId}` as any, {
          method: 'PUT',
          body: JSON.stringify({ role: role === 'editor' ? 'member' : role }),
        });
      } else {
  ...
  return (
    <>
      ...
      >
        ...
        {isWorkspaceBoard && <span className="hidden rounded-full bg-slate-100 px-1.5 py-0.5 text-xs text-slate-600 dark:bg-slate-700 dark:text-slate-300 sm:inline">workspace</span>}
      </button>
          ...
          <div className="relative flex max-h-[85vh] w-full max-w-[560px] flex-col rounded-xl border border-slate-200 bg-white shadow-xl dark:border-slate-800 dark:bg-slate-900">
            ...
            <div className="flex items-start justify-between border-b border-slate-200 px-5 py-4 dark:border-slate-800">
              <div>
                <h3 className="flex items-center gap-2 text-sm font-semibold text-slate-900 dark:text-white">
                  <Users className="h-4 w-4 text-slate-500" /> {isWorkspaceBoard ? 'Anggota workspace' : 'Undang anggota'}
                </h3>
                <p className="mt-1 text-xs text-slate-500 dark:text-slate-400">
                  {isWorkspaceBoard ? 'Akses board mengikuti workspace — owner bisa kelola semua.' : 'Kelola akses board — owner bisa ubah peran & hapus anggota.'}
                </p>
              </div>
            ...
            <div className="flex-1 overflow-auto">
              ...
              <div className="border-b border-slate-100 bg-slate-50/50 px-5 py-4 dark:border-slate-800 dark:bg-slate-800/30">
                <label className="mb-1.5 block text-xs font-medium text-slate-700 dark:text-slate-300">Undang via email {isWorkspaceBoard && <span className="font-normal text-slate-500">— akan masuk workspace</span>}</label>
                <div className="flex gap-2">
                  ...
                  >
                    ...
                    {isWorkspaceBoard && <option value="admin">Admin</option>}
                    {isWorkspaceBoard && <option value="member">Member</option>}
                  </select>
                  <button
                ...
                {!isOwner && <p className="mt-1.5 text-xs text-slate-500">Hanya owner/admin yang bisa mengundang.</p>}
              ...
              <div className="px-5 py-4">
                <h4 className="mb-3 flex items-center gap-1.5 text-xs font-semibold text-slate-600 dark:text-slate-400">
                  ...
                  {isWorkspaceBoard && <span className="ml-1 text-xs font-normal text-slate-500">— inherit ke semua board di workspace</span>}
                </h4>
                  ...
                  {members.map((m) => (
                    <li key={m.user_id} className="flex items-center gap-3 rounded-lg border border-slate-200 bg-white px-3 py-2.5 dark:border-slate-700 dark:bg-slate-800">
                      ...
                      {isOwner && m.role !== 'owner' ? (
                        ...
                        <span className={`inline-flex items-center gap-1 rounded-full border px-2 py-1 text-xs font-medium ${m.role === 'owner' ? 'border-amber-200 bg-amber-50 text-amber-700 dark:border-amber-900 dark:bg-amber-950 dark:text-amber-300' : m.role === 'editor' || m.role === 'member' ? 'border-slate-200 bg-slate-50 text-slate-700 dark:border-slate-600 dark:bg-slate-700 dark:text-slate-300' : 'border-slate-200 bg-white text-slate-500 dark:border-slate-600 dark:bg-slate-800 dark:text-slate-400'}`}>
